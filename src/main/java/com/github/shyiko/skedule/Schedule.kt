...
/**
 * An alternative syntax for cron.
 * Designed after GAE's [schedule](https://cloud.google.com/appengine/docs/standard/java/config/cronref#schedule_format).
 */
class Schedule private constructor(
    // "every" N ...
    private val interval: Interval? = null,
    // ("every"|ordinal) ...
    private val weeks: Set<Int> = WEEK_ALL,
    ...
    init {
        validate(!weeks.isEmpty() && weeks.all { it in WEEK_RANGE },
            { "Invalid week(s) (${weeks.filter { it !in WEEK_RANGE }})" })
        ...
        assert(interval != null || time != null)
    }
    ...
    private data class Interval(val length: Long, val unit: ChronoUnit, val time: TimeInterval? = null) {
        init {
    ...
    private val lazyString = lazy(LazyThreadSafetyMode.PUBLICATION, {
        when {
            interval != null -> {
            ...
            time != null -> {
                ...
                when {
                    ...
                    weeks == WEEK_ALL && weekDays == DAY_OF_WEEK_ALL ->
                        if (months != MONTH_ALL) "every day of ${months.joinSorted()} $time" else "every day $time"
                    weeks == WEEK_ALL && months == MONTH_ALL ->
                        "every ${weekDays.joinSorted()} $time"
                    else -> "${if (weeks == WEEK_ALL) "every" else weeks.joinToString(",")
    ...
    private fun <T: Enum<T>> Set<T>.joinSorted() = this.sorted().map { it.toString().toLowerCase() }.joinToString(",")
    ...
    companion object {
        ...
        private val H24 = TimeInterval(LocalTime.MIN, LocalTime.MAX)
        ...
        private fun parseUnit(unit: String) =
            UNIT_MAP[unit] ?: throw InvalidScheduleException("\"$unit\" isn't a valid unit")
        ...
        fun parse(schedule: String): Schedule =
            when {
                schedule.startsWith("every day ") -> {
                    ...
                    Schedule(time = parseTime(rawTime))
                }
                schedule.startsWith("every ") && schedule.length > "every ".length && schedule["every ".length].isDigit() -> {
                    ...
                    Schedule(interval = Interval(length, unit,
                        when {
                            spreadEvenlyOver24hPeriod -> H24
                            start != "" && end != "" -> TimeInterval(parseTime(start), parseTime(end))
                            else -> null
                        }
                    ))
                }
                    ...
                    Schedule(
                        weeks = if (day == "" || rawOrdinal == "every") WEEK_ALL else
        ...
        @JvmStatic
        fun from(start: LocalTime, end: LocalTime): IntervalScheduleBuilder = IntervalScheduleBuilder(start, end)
        @JvmOverloads
        @JvmStatic
        fun every(length: Long, unit: ChronoUnit, sync: Boolean = false): Schedule =
            Schedule(interval = Interval(length, unit, if (sync) H24 else null))

        class IntervalScheduleBuilder internal constructor(private val start: LocalTime, private val end: LocalTime) {
            fun every(length: Long, unit: ChronoUnit): Schedule =
                Schedule(interval = Interval(length, unit, TimeInterval(
                    start.truncatedTo(ChronoUnit.SECONDS), end.truncatedTo(ChronoUnit.SECONDS)
                )))
        }

        @JvmStatic
        fun at(time: LocalTime): TimeScheduleBuilder = TimeScheduleBuilder(time)

        class TimeScheduleBuilder internal constructor(private val time: LocalTime) {
            fun everyDay(): Schedule = Schedule(time = time)
            fun everyDay(months: Set<Month>): Schedule = Schedule(time = time, months = months)
            fun every(weekDays: Set<DayOfWeek>): Schedule = Schedule(time = time, weekDays = weekDays)
            fun every(weekDays: Set<DayOfWeek>, months: Set<Month>): Schedule =
                Schedule(time = time, weekDays = weekDays, months = months)
            fun nth(weeks: Set<Int>, weekDays: Set<DayOfWeek>): NthDayOfWeekScheduleBuilder =
                NthDayOfWeekScheduleBuilder(time, weeks, weekDays)
            fun nth(days: Set<Int>): NthDayOfMonthScheduleBuilder = NthDayOfMonthScheduleBuilder(time, days)
        }
        class NthDayOfWeekScheduleBuilder internal constructor(
            private val time: LocalTime,
            private val weeks: Set<Int>,
            private val weekDays: Set<DayOfWeek>
        ) {
            fun of(months: Set<Month>): Schedule =
                Schedule(time = time, weeks = weeks, weekDays = weekDays, months = months)
        }
        class NthDayOfMonthScheduleBuilder internal constructor(
            private val time: LocalTime,
            private val days: Set<Int>
        ) {
            fun of(months: Set<Month>): Schedule = Schedule(time = time, days = days, months = months)
        }

        inline fun validate(valid: Boolean, msg: () -> String) { if (!valid) throw InvalidScheduleException(msg()) }
    }

    private fun ZonedDateTime.withTime(value: LocalTime) =
        this.withHour(value.hour).withMinute(value.minute)

    private val ZonedDateTime.week: Int
        get() = (this.dayOfMonth + 6) / 7

    fun next(timestamp: ZonedDateTime): ZonedDateTime =
        when {
            interval != null -> {
            ...
            time != null -> {
                ...
                do {
                    ...
                    if (weeks != WEEK_ALL) {
                        val nextOrSameWeek = weeks.filter { it >= cursor.week }.min()
                            ?: weeks.min()!!
    ...
    fun iterate(timestamp: ZonedDateTime): Iterator<ZonedDateTime> {
        var cursor = timestamp
    ...
    override fun equals(other: Any?): Boolean = this === other ||
        (other?.javaClass == javaClass && lazyString.value == (other as Schedule).lazyString.value)
